functions {
  real[] one_comp_mm_elim_abs(real t,
                              real[] y,
                              real[] theta,
                              real[] x_r,
                              int[] x_i) {
    real dydt[1];
    real k_a = theta[1]; // Dosing rate in 1/day
    real K_m = theta[2]; // Michaelis-Menten constant in mg/L
    real V_m = theta[3]; // Maximum elimination rate in 1/day
    real D = x_r[1];
    real V = x_r[2];
    real dose = 0;
    real elim = (V_m / V) * y[1] / (K_m + y[1]);

    if (t > 0)
      dose = exp(- k_a * t) * D * k_a / V;

    dydt[1] = dose - elim;

    return dydt;
  }
}

transformed data {
  int N_t = 20;
  real times[N_t];
  real t0 = 0;
  real C0[1] = {0.0};
  // Dosing rate in 1/day
  // Michaelis-Menten constant in mg/L
  // Maximum elimination rate in 1/day
  real theta_sim[3];
  real x_r[2] = {30.0, 2.0}; // Total dosage in mg, Comparment volume in L
  int x_i[0];
  real C_sim[N_t, 1];
  real C_hat_sim[N_t];
  real<lower=0> k_a_sim; // Dosing rate in 1/day
  real<lower=0> K_m_sim; // Michaelis-Menten constant in mg/L
  real<lower=0> V_m_sim; // Maximum elimination rate in 1/day
  real<lower=0> sigma_sim;
  
  k_a_sim = fabs(cauchy_rng(0, 1));
  K_m_sim = fabs(cauchy_rng(0, 1));
  V_m_sim = fabs(cauchy_rng(0, 1));
  sigma_sim = fabs(cauchy_rng(0, 1));
  
  theta_sim = {k_a_sim, K_m_sim, V_m_sim};

  for (n in 1:N_t)
    times[n] = 0.5 * n;

  C_sim = integrate_ode_bdf(one_comp_mm_elim_abs, C0, t0, times, theta_sim, x_r, x_i);

  for (n in 1:N_t)
    C_hat_sim[n] = lognormal_rng(log(C_sim[n, 1]), sigma_sim);
}

parameters {
  real<lower=0> k_a; // Dosing rate in 1/day
  real<lower=0> K_m; // Michaelis-Menten constant in mg/L
  real<lower=0> V_m; // Maximum elimination rate in 1/day
  real<lower=0> sigma;
}

transformed parameters {
  real C[N_t, 1];
  {
    real theta[3] = {k_a, K_m, V_m};
    C = integrate_ode_bdf(one_comp_mm_elim_abs, C0, t0, times, theta, x_r, x_i);
  }
}

model {
  // Priors
  k_a ~ cauchy(0, 1);
  K_m ~ cauchy(0, 1);
  V_m ~ cauchy(0, 1);
  sigma ~ cauchy(0, 1);

  // Likelihood
  for (n in 1:N_t)
    C_hat_sim[n] ~ lognormal(log(C[n, 1]), sigma);
}


generated quantities {
    int<lower=0,upper=1> lt_sim[4];
    lt_sim[1] = k_a < k_a_sim;
    lt_sim[2] = K_m < K_m_sim;
    lt_sim[3] = V_m < V_m_sim;
    lt_sim[4] = sigma < sigma_sim;
}
